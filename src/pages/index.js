...
function MultipleChoiceItem(props) {
    ...
    return(
        <Button style={{border: `none`, background: `rgba(0, 0, 0, 0)`, color: `#00F`}} onClick={updateScores}>
            {props.children}
        </Button>
    )
}
...
function QuestionThumbnail({question, answer, questionIndex, index, goToQuestion, closeFunction}) {
    ...
    return(
      <Button className="m-3 view img-button comic-strip-page-thumbnail" aria-label={`Question ${index + 1}: ${question.internal.content} (${(answer > -1) ? (question.frontmatter.choices[answer]) : "No Answer"})`} onClick={changeQuestion}>
    ...
    return (
      <>
        ...
        <Modal.Body className="px-0">
          ...
          <div className="ausome-blogs justify-content-center p-3">
  ...
  render() {
    ...
    return(
      ...
      <div className={"table-background-" + this.state.currentTableBackground.toLowerCase().replace(/ /g, "-")} style={{textAlign: 'center'}}>
        <div className="ausome-blogs p-3" style={{textAlign: 'center'}}>
